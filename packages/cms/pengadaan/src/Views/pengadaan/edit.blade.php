				...
				<div class="box-body">
					...
					<div class="form-group {{ ($errors->first('jenis_pengajuan')) ? 'has-error' : '' }}">
						...
						<select name="jenis_pengajuan" class="form-control" id="fjenis_pengajuan">
							...
							<option value="1" {{ ($pengadaan->jenis_pengajuan=='1') ? 'selected' : '' }}>Pengadaan</option>
							<option value="2" {{ ($pengadaan->jenis_pengajuan=='2') ? 'selected' : '' }}>Perbaikan</option>
							<option value="3" {{ ($pengadaan->jenis_pengajuan=='3') ? 'selected' : '' }}>Pergantian</option>
						</select>
						@if($errors->has('jenis_pengajuan'))										
							<span class="help-block">{{ $errors->first('jenis_pengajuan') }}</span>
						@endif
					...
					<div class="wrap-item-pengajuan">
						...
						<div class="box-body item-pengajuan" id="item-pengajuan{{ $item_key }}">
							...
							<div class="form-group">
								...
								<input type="text" class="form-control" id="fspesifikasi_barang" name="pengadaan[{{ $item_key }}][spesifikasi_barang]" value="{{ $item->spesifikasi_barang }}" required>
							</div>

							<div class="form-group">
								<label for="furaian_barang">Uraian Barang</label>
								<input type="text" class="form-control" id="furaian_barang" name="pengadaan[{{ $item_key }}][uraian_barang]" value="{{ $item->uraian_barang }}" required>
							</div>

							<div class="form-group">
								<label for="fketerangan">Keterangan</label>
								<textarea class="form-control" placeholder="Keterangan" id="fketerangan" name="pengadaan[{{ $item_key }}][keterangan]" required >{{ $item->keterangan }}</textarea>
							</div>
	...
	$(document).ready(function(){
		$('body').on('click', '.add-item', function(evt) {
			...
			_html = '<div class="box-body item-pengajuan" id="item-pengajuan'+_new_item_row+'">'+
				...
				'<div class="form-group">'+
					'<label for="fspesifikasi_barang">Spesifikasi Barang</label>'+
					'<input type="text" class="form-control" id="fspesifikasi_barang" name="pengadaan['+_new_item_row+'][spesifikasi_barang]" value="" required>'+
				'</div>'+
				'<div class="form-group">'+
					'<label for="furaian_barang">Uraian Barang</label>'+
					'<input type="text" class="form-control" id="furaian_barang" name="pengadaan['+_new_item_row+'][uraian_barang]" value="" required>'+
				'</div>'+
				'<div class="form-group">'+
					'<label for="fketerangan">Keterangan</label>'+
					'<textarea class="form-control" placeholder="Keteranga" id="fketerangan" name="pengadaan['+_new_item_row+'][keterangan]" required></textarea>'+
				'</div>'+
				'<div class="form-group">'+
