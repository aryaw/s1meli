      ...
      <div class="col-md-12">
        ...
        <div class="box box-primary">
          ...
          <!-- /.box-header -->
			...
			<!-- form start -->
			{!! Form::open(['route' => ['cms.pengadaan.updatebyrole', 'id'=>$pengadaan->id], 'role'=>'form', 'autocomplete'=>'off']) !!}	
				<div class="box-body">
					<div class="form-group {{ ($errors->first('pemohon')) ? 'has-error' : '' }}">
						<label for="fpemohon">Pemohon</label>
						<input type="text" class="form-control" id="fnama_barang" name="" value="{{ $pengadaan->user->full_name }}" readonly>
					</div>

					<div class="form-group {{ ($errors->first('nomor_laporan')) ? 'has-error' : '' }}">
						<label for="fnomor_laporan">No. Laporan</label>
						<textarea class="form-control" placeholder="No. Laporan" id="fnomor_laporan" name="" readonly>{{ $pengadaan->nomor_laporan }}</textarea>
					</div>

					<div class="form-group {{ ($errors->first('tgl_pengajuan')) ? 'has-error' : '' }}">
						<label for="ftgl_pengajuan">Tanggal Pengajuan</label>
						<input type="text" class="form-control datepicker" id="ftgl_pengajuan" name="" value="{{ $pengadaan->pengajuan }}" readonly>
					</div>
				...
				<!-- /.box-body -->																		
				@if($btn == 'kepsek' && $pengadaan->related_history->approve_kepsek=='0')
				...
				@if($btn == 'wakasek' && $pengadaan->related_history->approve_wakasek=='0')
