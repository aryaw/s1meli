				...
				<div class="box-body">
					...
					<div class="form-group {{ ($errors->first('jenis_pengajuan')) ? 'has-error' : '' }}">
						...
						<select name="jenis_pengajuan" class="form-control" id="fjenis_pengajuan" readonly="true">
							...
							<option value="1" {{ ($penerimaan->jenis_pengajuan=='1') ? 'selected' : '' }}>Pengadaan</option>
							<option value="2" {{ ($penerimaan->jenis_pengajuan=='2') ? 'selected' : '' }}>Perbaikan</option>
							<option value="3" {{ ($penerimaan->jenis_pengajuan=='3') ? 'selected' : '' }}>Pergantian</option>
						</select>
						@if($errors->has('jenis_pengajuan'))										
							<span class="help-block">{{ $errors->first('jenis_pengajuan') }}</span>
						@endif
					...
					<div class="form-group {{ ($errors->first('tgl_pengajuan')) ? 'has-error' : '' }}">
						...
						<input type="readonly" class="form-control datepicker" id="ftgl_pengajuan" name="tgl_pengajuan" value="{{ $penerimaan->pengajuan }}" required readonly="true">
					...
					<div class="wrap-item-pengajuan">
						...
						<div class="box-body item-pengajuan" id="item-pengajuan{{ $item_key }}">
							<div class="form-group">
								...
								<input type="readonly" class="form-control" id="fnama_barang" name="penerimaan[{{ $item_key }}][nama_barang]" value="{{ $item->nama_barang }}" required readonly="true">
							</div>

							<div class="form-group">
								<label for="fspesifikasi_barang">Spesifikasi Barang</label>
								<input type="readonly" class="form-control" id="fspesifikasi_barang" name="penerimaan[{{ $item_key }}][spesifikasi_barang]" value="{{ $item->spesifikasi_barang }}" required readonly="true">
							</div>

							<div class="form-group">
								<label for="furaian_barang">Uraian Barang</label>
								<input type="readonly" class="form-control" id="furaian_barang" name="penerimaan[{{ $item_key }}][uraian_barang]" value="{{ $item->uraian_barang }}" required readonly="true">
							</div>

							<div class="form-group">
								<label for="fketerangan">Keterangan</label>
								<textarea class="form-control" placeholder="Keterangan" id="fketerangan" name="penerimaan[{{ $item_key }}][keterangan]" required readonly="true">{{ $item->keterangan }}</textarea>
							</div>
						</div>
						@endforeach
						@endif
