			...
			{!! Form::open(['route' => ['cms.kerusakan.update', 'id'=>$kerusakan->id], 'role'=>'form', 'autocomplete'=>'off']) !!}	
				<div class="box-body">
					...
					<div class="form-group {{ ($errors->first('nomor_laporan')) ? 'has-error' : '' }}">
						...
						<textarea class="form-control" placeholder="No. Laporan" id="fnomor_laporan" name="nomor_laporan" required>{{ $kerusakan->nomor_laporan }}</textarea>
					...
					<div class="form-group {{ ($errors->first('tgl_pengajuan')) ? 'has-error' : '' }}">
						...
						<input type="text" class="form-control datepicker" id="ftgl_pengajuan" name="tgl_pengajuan" value="{{ $kerusakan->pengajuan }}" required>
					...
					<div class="wrap-item-pengajuan">
						...
						@php
							$item_key = 1;
						@endphp
						@if(isset($kerusakan->item_pengadaan))
						@foreach($kerusakan->item_pengadaan as $key => $item)
						@php
							$item_key = $item->id+1;
						@endphp
						<div class="box-body item-pengajuan" id="item-pengajuan{{ $item_key }}">
							<div class="form-group">
								...
								<input type="text" class="form-control" id="fnama_barang" name="kerusakan[{{ $item_key }}][nama_barang]" value="{{ $item->nama_barang }}" required>
							</div>

							<div class="form-group">
								<label for="fspesifikasi_barang">Spesifikasi Barang</label>
								<textarea class="form-control" placeholder="Spesifikasi Barang" id="fketerangan" name="kerusakan[{{ $item_key }}][spesifikasi_barang]" >{{ $item->spesifikasi_barang }}</textarea>
							</div>

							<div class="form-group">
								<label for="furaian_barang">Uraian Barang</label>
								<input type="text" class="form-control" id="furaian_barang" name="kerusakan[{{ $item_key }}][uraian_barang]" value="{{ $item->uraian_barang }}" >
							</div>

							<div class="form-group">
								<label for="furaian_barang">Qty</label>
								<input type="text" class="form-control" id="fqty" name="kerusakan[{{ $item_key }}][qty]" value="{{ $item->qty }}" >
							</div>

							<div class="form-group">
								<label for="fketerangan">Keterangan</label>
								<textarea class="form-control" placeholder="Keterangan" id="fketerangan" name="kerusakan[{{ $item_key }}][keterangan]"  >{{ $item->keterangan }}</textarea>
							</div>
