      ...
      <div class="col-md-12">
        ...
        <div class="box box-primary">
          ...
          <!-- /.box-header -->
			...
			<!-- form start -->
			{!! Form::open(['route' => ['cms.kerusakan.update', 'id'=>$kerusakan->id], 'role'=>'form', 'autocomplete'=>'off']) !!}	
				<div class="box-body">
					<div class="form-group {{ ($errors->first('pemohon')) ? 'has-error' : '' }}">
						...
						<select name="pemohon" class="form-control" id="fpemohon" readonly="true">
							...
							@foreach($user as $pemohon)
								<option value="{{ $pemohon->id }}" {{ ($kerusakan->user_id==$pemohon->id) ? 'selected' : '' }}>{{ $pemohon->full_name }}</option>
							@endforeach
						</select>
					...
					<div class="form-group {{ ($errors->first('jenis_pengajuan')) ? 'has-error' : '' }}">
						...
						<select name="jenis_pengajuan" class="form-control" id="fjenis_pengajuan" readonly="true">
							...
							<option value="1" {{ ($kerusakan->jenis_pengajuan=='1') ? 'selected' : '' }}>Pengadaan</option>
							<option value="2" {{ ($kerusakan->jenis_pengajuan=='2') ? 'selected' : '' }}>Perbaikan</option>
							<option value="3" {{ ($kerusakan->jenis_pengajuan=='3') ? 'selected' : '' }}>Pergantian</option>
						</select>
						@if($errors->has('jenis_pengajuan'))										
							<span class="help-block">{{ $errors->first('jenis_pengajuan') }}</span>
						@endif
					...
					<div class="form-group {{ ($errors->first('tgl_pengajuan')) ? 'has-error' : '' }}">
						...
						<input type="readonly" class="form-control datepicker" id="ftgl_pengajuan" name="tgl_pengajuan" value="{{ $kerusakan->pengajuan }}" required readonly="true">
					...
					<div class="wrap-item-pengajuan">
						...
						<div class="box-body item-pengajuan" id="item-pengajuan{{ $item_key }}">
							<div class="form-group">
								...
								<input type="hidden" class="form-control" id="fitem" name="kerusakan[{{ $item_key }}][item]" value="{{ $item->id }}" required>
								<input type="readonly" class="form-control" id="fnama_barang" name="kerusakan[{{ $item_key }}][nama_barang]" value="{{ $item->nama_barang }}" required readonly="true">
							</div>

							<div class="form-group">
								<label for="fspesifikasi_barang">Spesifikasi Barang</label>
								<input type="readonly" class="form-control" id="fspesifikasi_barang" name="kerusakan[{{ $item_key }}][spesifikasi_barang]" value="{{ $item->spesifikasi_barang }}" required readonly="true">
							</div>

							<div class="form-group">
								<label for="furaian_barang">Uraian Barang</label>
								<input type="readonly" class="form-control" id="furaian_barang" name="kerusakan[{{ $item_key }}][uraian_barang]" value="{{ $item->uraian_barang }}" required readonly="true">
							</div>

							<div class="form-group">
								<label for="fketerangan">Keterangan</label>
								<textarea class="form-control" placeholder="Keterangan" id="fketerangan" name="kerusakan[{{ $item_key }}][keterangan]" required readonly="true">{{ $item->keterangan }}</textarea>
							</div>
						</div>
						@endforeach
						@endif
